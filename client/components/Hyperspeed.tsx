...
const Hyperspeed = ({
  ...
  useEffect(() => {
    ...
    class CarLights {
      ...
      constructor(webgl: App, options: any, colors: any, speed: [number, number], fade: THREE.Vector2) {
        this.webgl = webgl;
        this.options = options;
        this.colors = colors;
        this.speed = speed;
      ...
      init() {
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          let radius = random(options.carLightsRadius) as number;
          let length = random(options.carLightsLength) as number;
          let speed = random(this.speed) as number;

          let carLane = i % options.lanesPerRoad;
          let laneX = carLane * laneWidth - options.roadWidth / 2 + laneWidth / 2;

          let carWidth = random(options.carWidthPercentage) as number * laneWidth;
          let carShiftX = random(options.carShiftX) as number * laneWidth;
          laneX += carShiftX;

          let offsetY = random(options.carFloorSeparation) as number + radius * 1.3;

          let offsetZ = -random(options.length) as number;
    ...
    class LightsSticks {
      ...
      init() {
        ...
        for (let i = 0; i < totalSticks; i++) {
          let width = random(options.lightStickWidth) as number;
          let height = random(options.lightStickHeight) as number;
