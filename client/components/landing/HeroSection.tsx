...
export const HeroSection = memo(function HeroSection({ onGetStartedClick }: HeroSectionProps) {
  ...
  // Check if we should show heavy effects - disable on mobile for performance
  const isMobile = useMemo(() => deviceCapability?.isMobile || false, [deviceCapability]);

  // Show Hyperspeed only on desktop with WebGL support
  const shouldShowHyperspeed = useMemo(() => {
    // Disable Hyperspeed on mobile for better performance
  ...
  useEffect(() => {
    ...
    // Longer delay on low-end devices
  ...
  return (
    ...
    >
      {/* Hyperspeed background effect - only on desktop */}
      ...
      {/* Indonesian cultural elements - only on desktop for performance */}
      {!isMobile && (
        <>
      ...
      <div className="relative z-20 text-center max-w-4xl mx-auto">
        ...
        {/* CTA Buttons - simplified without inline mouse handlers */}
        <ScrollReveal delay={0.4} duration={0.7} distance={25}>
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
            <button
              onClick={onGetStartedClick || handleTryNow}
              className="px-8 py-3 sm:px-10 sm:py-4 bg-primary text-primary-foreground font-semibold font-heading text-base sm:text-lg rounded-lg hover-glow button-batik-hover focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background cursor-pointer transition-all duration-300 hover:scale-[1.03] active:scale-[0.98]"
            ...
            <button
              ...
              className="px-8 py-3 sm:px-10 sm:py-4 border-2 border-indonesian-gold/60 text-foreground font-semibold font-heading text-base sm:text-lg rounded-lg hover-glow button-batik-hover focus:outline-none focus:ring-2 focus:ring-indonesian-gold focus:ring-offset-2 focus:ring-offset-background transition-all duration-300 hover:scale-[1.03] active:scale-[0.98]"
